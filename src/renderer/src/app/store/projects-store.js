...
export const useProjectsStore = create((set, get) => ({
        ...
        try {
            ...
            set((state) => ({
                ...
                expandedDirectories: {
                    ...
                    [projectId]: removeMatchingPaths(state.expandedDirectories[projectId] ?? [], relativePath),
                },
                fullyLoadedTreeProjects: {
                    ...state.fullyLoadedTreeProjects,
                    [projectId]: false,
    ...
    hydrate: async (preferredProjectId = null) => {
        try {
            ...
            const nextActiveProjectId = projects.find((project) => project.id === preferredProjectId ||
                project.id === currentActiveProjectId)?.id ??
                projects[0]?.id ??
                null;
            set({
                activeProjectId: nextActiveProjectId,
    ...
    loadEntireProjectTree: async (projectId) => {
        ...
        while (queue.length > 0) {
            ...
            if (!(parentKey in currentTree)) {
                await loadDirectory(projectId, currentNode.relativePath, set, get);
            }
    ...
    refreshProjectTree: async (projectId) => {
        ...
        try {
            const entries = await Promise.all(parentPaths.map(async (parentRelativePath) => ({
    ...
    renameEntry: async (projectId, relativePath, nextName) => {
        try {
            ...
            set((state) => ({
                ...
                expandedDirectories: {
                    ...state.expandedDirectories,
                    [projectId]: renameMatchingPaths(state.expandedDirectories[projectId] ?? [], relativePath, entry.relativePath),
                },
                fullyLoadedTreeProjects: {
                    ...state.fullyLoadedTreeProjects,
                    [projectId]: false,
    ...
    removeProject: async (projectId) => {
        try {
            ...
            set((state) => ({
                ...
                error: null,
                expandedDirectories: omitProjectKey(state.expandedDirectories, projectId),
                fullyLoadedTreeProjects: omitProjectKey(state.fullyLoadedTreeProjects, projectId),
                projects,
        ...
        set((state) => ({
            expandedDirectories: {
                ...
                [projectId]: isExpanded
                    ? expandedDirectories.filter((path) => path !== node.relativePath)
...
function getParentKey(parentRelativePath) {
    return parentRelativePath ?? ROOT_NODE_KEY;
}
async function loadDirectory(projectId, parentRelativePath, set, get) {
    ...
    try {
        ...
        set((state) => ({
            loadingNodeKeys: state.loadingNodeKeys.filter((key) => key !== nodeKey),
        }));
    }
}
...
function omitProjectKey(record, projectId) {
    return Object.fromEntries(Object.entries(record).filter(([key]) => key !== projectId));
}
function removeMatchingPaths(paths, relativePath) {
    return paths.filter((path) => path !== relativePath && !path.startsWith(`${relativePath}/`));
}
function renameMatchingPaths(paths, previousPath, nextPath) {
