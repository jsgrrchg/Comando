...
import { activatePane, appendTerminalOutput, attachTabToPane, closeOtherWorkspaceTabs, closeWorkspacePane, closeWorkspaceTab, closeWorkspaceTabsForProjectPath, closeWorkspaceTabsToRight, createDefaultWorkspaceState, markTerminalExited, moveActiveTabBetweenPanes, moveWorkspaceTabBetweenPanes, removeProjectTabs, renameWorkspaceTabsForProjectPath, replaceFileDocument, resizeSplit, selectPaneTab, setFileTabLoading, setFileTabLoadError, setFileTabSaving, setTerminalLaunchError, setTerminalSessionReady, splitPaneInDirection, updateChatDraft, updateFileDraft as applyFileDraft, workspaceStateFromSnapshot, workspaceStateToSnapshot, } from "../workspace/tree";
export const useWorkspaceStore = create((set, get) => ({
    ...
    closeTabsForProjectPath: async (projectId, relativePath, kind) => {
        set((state) => ({
            ...closeWorkspaceTabsForProjectPath(state, projectId, relativePath, kind),
            error: null,
        }));
        await persistWorkspaceState(get);
    },
    ...
    handleTerminalExit: (event) => {
        set((state) => ({
            ...markTerminalExited(state, event.sessionId, event.exitCode, event.signalCode),
        }));
    },
    hydrate: async () => {
        try {
            ...
            set({
                ...workspaceStateFromSnapshot(snapshot, createHydratedRuntimeTabs(snapshot)),
                error: null,
                hydrated: true,
            });
    ...
    openFileTab: async (projectId, relativePath) => {
        try {
            const existingTab = findExistingFileTab(get(), projectId, relativePath);
            if (existingTab) {
    ...
    refreshProjectTabs: async (projectId) => {
        const fileTabs = Object.values(get().tabsById).filter((tab) => tab.kind === "file" && tab.projectId === projectId);
    ...
    removeProjectTabs: async (projectId) => {
        ...
        await Promise.all(terminalSessionIds.map((sessionId) => safeCloseTerminal(sessionId)));
        set((state) => ({
    ...
    renameTabsForProjectPath: async (projectId, previousRelativePath, nextRelativePath, kind) => {
        set((state) => ({
            ...renameWorkspaceTabsForProjectPath(state, projectId, previousRelativePath, nextRelativePath, kind),
            error: null,
        }));
        await persistWorkspaceState(get);
    },
    ...
    saveFileTab: async (tabId) => {
        ...
        try {
            ...
            set((state) => ({
                ...setFileTabSaving(state, tabId, false, error instanceof Error
                    ? error.message
...
function createHydratedRuntimeTabs(snapshot) {
    return Object.fromEntries(snapshot.tabs.map((tab) => {
        if (tab.kind === "chat") {
            return [tab.id, tab];
        }
        if (tab.kind === "review") {
...
async function hydrateRuntimeTabs(snapshot, get, set) {
    await Promise.all(snapshot.tabs.map(async (tab) => {
        if (tab.kind === "chat") {
            try {
                const persistedSession = await getComandoApi().getChatSessionState(tab.sessionId);
                if (!persistedSession) {
                    return;
                }
                set((state) => ({
...
async function persistWorkspaceState(get) {
    try {
        const state = get();
        await getComandoApi().saveWorkspaceSnapshot(workspaceStateToSnapshot(state));
    }
    catch (error) {
        console.error(error);
    }
}
async function reloadFileTab(get, set, tabId) {
    const tab = get().tabsById[tabId];
    ...
    try {
        ...
        set((state) => ({
            ...setFileTabLoadError(state, tabId, error instanceof Error
                ? error.message
...
async function bootTerminalSession(get, set, tabId) {
    const tab = get().tabsById[tabId];
    if (!tab || tab.kind !== "terminal") {
        return;
    }
    try {
        ...
        set((state) => ({
            ...setTerminalLaunchError(state, tabId, error instanceof Error
                ? error.message
...
async function safeCloseTerminal(sessionId) {
    try {
...
async function safeCloseAiSession(sessionId) {
    try {
...
function findExistingFileTab(state, projectId, relativePath) {
    return (Object.values(state.tabsById).find((tab) => tab.kind === "file" &&
        tab.projectId === projectId &&
        tab.relativePath === relativePath) ?? null);
}
function findExistingReviewTab(state, sessionId) {
    return (Object.values(state.tabsById).find((tab) => tab.kind === "review" && tab.sessionId === sessionId) ?? null);
}
function findPaneIdByTabId(state, tabId) {
