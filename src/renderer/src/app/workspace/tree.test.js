...
import { attachTabToPane, closeOtherWorkspaceTabs, closeWorkspacePane, closeWorkspaceTabsForProjectPath, closeWorkspaceTabsToRight, createDefaultWorkspaceState, moveActiveTabBetweenPanes, moveWorkspaceTabBetweenPanes, renameWorkspaceTabsForProjectPath, splitPaneInDirection, } from "./tree";
function makeChatTab(id) {
    return {
...
function makeFileTab(id, relativePath, projectId = "project-1") {
    return {
        createdAt: "2026-04-12T00:00:00.000Z",
        document: {
...
describe("workspace tree helpers", () => {
    it("splits a pane and activates the new pane", () => {
        const state = splitPaneInDirection(createDefaultWorkspaceState(), "pane-root", "right", {
            paneId: "pane-2",
            splitId: "split-1",
        });
    ...
    it("moves the active tab to the next pane", () => {
        const splitState = splitPaneInDirection(createDefaultWorkspaceState(), "pane-root", "right", {
            paneId: "pane-2",
            splitId: "split-1",
        });
    ...
    it("moves a specific inactive tab to the previous pane", () => {
        ...
        const withFirstTab = attachTabToPane(splitState, "pane-root", makeChatTab("tab-1"));
        const withSecondTab = attachTabToPane(withFirstTab, "pane-root", makeChatTab("tab-2"));
        const withThirdTab = attachTabToPane(withSecondTab, "pane-2", makeChatTab("tab-3"));
        const moved = moveWorkspaceTabBetweenPanes(withThirdTab, "tab-2", "next");
    ...
    it("closes other tabs in the same pane", () => {
        const withFirstTab = attachTabToPane(createDefaultWorkspaceState(), "pane-root", makeChatTab("tab-1"));
        const withSecondTab = attachTabToPane(withFirstTab, "pane-root", makeChatTab("tab-2"));
        const withThirdTab = attachTabToPane(withSecondTab, "pane-root", makeChatTab("tab-3"));
    ...
    it("closes a pane and merges its tabs into the fallback pane", () => {
        ...
        const withFirstTab = attachTabToPane(splitState, "pane-root", makeChatTab("tab-1"));
        const withSecondTab = attachTabToPane(withFirstTab, "pane-2", makeChatTab("tab-2"));
    ...
    it("closes every file tab that matches a deleted directory", () => {
        const withReadme = attachTabToPane(createDefaultWorkspaceState(), "pane-root", makeFileTab("tab-1", "docs/readme.md"));
        const withGuide = attachTabToPane(withReadme, "pane-root", makeFileTab("tab-2", "docs/guides/getting-started.md"));
        const withOtherFile = attachTabToPane(withGuide, "pane-root", makeFileTab("tab-3", "src/index.ts"));
        const closed = closeWorkspaceTabsForProjectPath(withOtherFile, "project-1", "docs", "directory");
        expect(Object.keys(closed.tabsById).sort()).toEqual(["tab-3"]);
    });
    it("renames every matching open tab when a directory moves", () => {
        const withReadme = attachTabToPane(createDefaultWorkspaceState(), "pane-root", makeFileTab("tab-1", "docs/readme.md"));
        const withGuide = attachTabToPane(withReadme, "pane-root", makeFileTab("tab-2", "docs/guides/getting-started.md"));
        const renamed = renameWorkspaceTabsForProjectPath(withGuide, "project-1", "docs", "knowledge-base", "directory");
        ...
        expect(readmeTab.document?.relativePath).toBe("knowledge-base/readme.md");
        expect(readmeTab.title).toBe("readme.md");
        expect(guideTab.relativePath).toBe("knowledge-base/guides/getting-started.md");
        expect(guideTab.document?.absolutePath).toContain("knowledge-base/guides/getting-started.md");
    });
});
