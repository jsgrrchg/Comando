...
export function resolveCodexRuntime(settings) {
    ...
    return {
        ...
        status: {
            ...
            command: null,
            message: "No se encontró un runtime ACP compatible. Esta integración necesita `codex-acp` o un binario equivalente que implemente ACP.",
            runtimeId: "codex",
            source: null,
            state: "missing",
        },
...
function resolveCandidate(candidate, source) {
...
function createReadyStatus(command, source) {
    return {
        checkedAt: new Date().toISOString(),
        command,
        message: null,
...
function createResolvedCommand(executable, source) {
...
function createIncompatibleResolvedCommand(executable, source) {
    return {
        args: [],
        command: executable,
        executable,
        status: {
            checkedAt: new Date().toISOString(),
            command: executable,
            message: "Se encontró `codex`, pero este CLI expone App Server/MCP y no un runtime ACP. La integración actual de Comando todavía usa ACP.",
            runtimeId: "codex",
            source,
            state: "error",
        },
...
function buildRuntimeCommandSpec(executable) {
    return {
        args: [],
        command: executable,
        executable,
...
function isAcpCompatibleExecutable(executable) {
...
function resolveFromPath(command) {
    ...
    for (const entry of pathEntries) {
        for (const ext of pathextEntries) {
            const candidate = path.join(entry, process.platform === "win32" &&
