...
void app.whenReady().then(() => {
    ...
    registerIpcHandlers({
        ...
        getSnapshot: () => {
            if (!bootstrapSnapshot) {
                throw new Error("The initial bootstrap snapshot is not available yet.");
            }
    ...
    const mainWindow = createMainWindow(persistenceService.loadSnapshot().windowState);
...
function broadcastProjectTreeInvalidation(payload) {
    for (const window of BrowserWindow.getAllWindows()) {
        window.webContents.send(IPC_EVENTS.projectTreeInvalidated, payload);
    }
}
function attachWindowPersistence(window) {
    let timeout = null;
...
function broadcastAiRuntimeStatus(payload) {
    for (const window of BrowserWindow.getAllWindows()) {
        window.webContents.send(IPC_EVENTS.aiRuntimeStatus, payload);
    }
}
function broadcastAiSessionSnapshot(payload) {
    for (const window of BrowserWindow.getAllWindows()) {
