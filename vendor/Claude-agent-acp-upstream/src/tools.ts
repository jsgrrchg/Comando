...
function stripTrailingUsage(text: string): string {
  ...
  const openingTagIndex = trimmedEnd.lastIndexOf(
    "<usage>",
    trimmedEnd.length - closingTag.length,
  );
  if (openingTagIndex === -1) {
    return text;
  }

  // The newline is part of the trailer only when it immediately precedes it.
  const trailerStart =
